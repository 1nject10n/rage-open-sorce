...
mp.events.add("render", () => {
    if (streetName && zoneName) {
        ...
        if (useSpeedo && vehicle) {
            ...
            //Fuel
            var fuel = round(vehicle.getVariable('Vehicle.Fuel'), 1);
            if(!isNaN(fuel)){
                drawText(`~b~${fuel}`, [minimap.rightX + 0.055, minimap.bottomY - 0.095], 4, [255, 255, 255, 255], 0.6); 
                drawText(`Liter`, [minimap.rightX + 0.08, minimap.bottomY - 0.086], 4, [255, 255, 255, 255], 0.4);  
            }
            //Km
            var km = round(vehicle.getVariable('Vehicle.Km'), 1);
            if(!isNaN(km)){
        ...
        //Servername + Version
        drawText('Opensource RageMP', [minimap.rightX + 0.01, minimap.bottomY - 0.03], 4, [255, 255, 255, 255], 0.4);
    }
});
